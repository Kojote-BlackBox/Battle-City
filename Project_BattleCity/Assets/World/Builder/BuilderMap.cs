    ...
    public class BuilderMap
    {
        ...
        public void InitializeBaseLayer(TileData tileData)
        {
            ...
            for (int y = 0; y < map.rows; y++)
            {
                ...
                {
                    map.map[x, y, layer] = tiling(tilePattern, new Vector2(x, y));
                }
            }
        ...
        {
            GameObject cullingTile = GameObject.Instantiate(map.tilePrefab, map.transform);
            ...
            cullingTile.transform.SetParent(map.transform);
        ...
        {
            GameObject cullingTile = GameObject.Instantiate(tilePattern, map.transform);
        ...
        */
        ...
        {
            if (x < map.map.GetLength(0) && x >= 0 && y < map.map.GetLength(1) && y >= 0)
            {
                return false;
            }
            else
        ...
        public List<GameObject> GetAllNeighborTiles(int x, int y, int z)
        {
            ...
            for (int verticalOffset = -1; verticalOffset < 2; verticalOffset++)
            {
                for (int horizontalOffset = -1; horizontalOffset < 2; horizontalOffset++)
                {
                    if (NotOutOfMap(x + horizontalOffset, y + verticalOffset))
                    {
                        if (map.map[x + horizontalOffset, y + verticalOffset, z] != null)
                        {
                            neighbors.Add(map.map[x + horizontalOffset, y + verticalOffset, z]);
                        }
                    }
                }
            }

            neighbors.Remove(map.map[x, y, z]);
            return neighbors;
        }
        ...
        public Vector2 GetBorderedDirection(int x, int y)
        {
            ...
            else if (x == (map.map.GetLength(0) - 1) && y == (map.map.GetLength(1) - 1))
            {
                return new Vector2(-1.0f, -1.0f);
            }
            else if (x == 0 && y == (map.map.GetLength(1) - 1))
            {
                return new Vector2(1.0f, -1.0f);
            }
            else if (x == (map.map.GetLength(0) - 1) && y == 0)
            {
                return new Vector2(-1.0f, 1.0f);
            ...
            else if (x == (map.map.GetLength(0) - 1))
            {
                return new Vector2(-1.0f, 0.0f);
            }
            else if (y == 0)
        ...
        public List<GameObject> UpdateRiverBack(Vector2 direction, GameObject position)
        {
            ...
            if (position != null)
            {
                ...
                if (direction.x == 1 && direction.y == 1)
                {
                    if (xPosition - 1 > 0)
                    {
                        if (map.map[xPosition - 1, yPosition, 1] == null)
                        {
                            returnList.Add(map.map[xPosition - 1, yPosition, 0]);
                        }
                    }

                    if (yPosition - 1 >= 0)
                    {
                        if (map.map[xPosition, yPosition - 1, 1] == null)
                        {
                            returnList.Add(map.map[xPosition, yPosition - 1, 0]);
                        }
                    }
                ...
                {
                    if (xPosition + 1 < map.map.GetLength(0))
                    {
                        if (map.map[xPosition + 1, yPosition, 1] == null)
                        {
                            returnList.Add(map.map[xPosition + 1, yPosition, 0]);
                    ...
                    if (yPosition + 1 < map.map.GetLength(1))
                    {
                        if (map.map[xPosition, yPosition + 1, 1] == null)
                        {
                            returnList.Add(map.map[xPosition, yPosition + 1, 0]);
                ...
                else if (direction.x == 0 && direction.y == 1)
                {
                    ...
                    if (xPosition + 1 < map.map.GetLength(0) && yPosition - 1 >= 0)
                    {
                        if (map.map[xPosition + 1, yPosition - 1, 1] == null)
                        {
                            returnList.Add(map.map[xPosition + 1, yPosition - 1, 0]);
                        }
                    }
                ...
                else if (direction.x == 0 && direction.y == -1)
                {
                    ...
                    if (xPosition + 1 < map.map.GetLength(0) && yPosition + 1 < map.map.GetLength(1))
                    {
                        if (map.map[xPosition + 1, yPosition + 1, 1] == null)
                        {
                            returnList.Add(map.map[xPosition + 1, yPosition + 1, 0]);
                    ...
                    if (xPosition - 1 >= 0 && yPosition + 1 < map.map.GetLength(1))
                    {
                        if (map.map[xPosition - 1, yPosition + 1, 1] == null)
                        {
                            returnList.Add(map.map[xPosition - 1, yPosition + 1, 0]);
        ...
        public GameObject CreateWaterTile(GameObject referenceTile, Vector2 position)
        {
            if (NotOutOfMap((int)position.x, (int)position.y))
            {
                ...
                GameObject newWaterCullingTile = GameObject.Instantiate(referenceTile, map.transform);
                GameObject newWaterTile = newWaterCullingTile.GetComponent<Culling>().tile;

                newWaterCullingTile.transform.SetParent(this.map.transform);
                ...
                map.map[(int)position.x, (int)position.y, 1] = newWaterTile;
            ...
            for (int verticalOffset = -1; verticalOffset < 2; verticalOffset++)
            {
                for (int horizontalOffset = -1; horizontalOffset < 2; horizontalOffset++)
                {
                    ...
                    if (NotOutOfMap(x + horizontalOffset, y + verticalOffset))
                    {
                        ...
                        if (map.map[x + horizontalOffset, y + verticalOffset, 1] == null)
                        {
                            neighbors.Add(map.map[x + horizontalOffset, y + verticalOffset, 0]);
                        }
                    }
                }
            }
            neighbors.Remove(map.map[x, y, 0]);

            return neighbors;
        }

        /* 
        * �bergang zum Land machen
        ...
        */
        ...
        {
            // Set Water Border
            for (int x = 0; x < this.map.columns; x++)
            {
                SetNewWaterTile(x, 0);
                SetNewWaterTile(x, (this.map.rows - 1));
            }

            for (int y = 0; y < this.map.rows; y++)
            {
                SetNewWaterTile(0, y);
                SetNewWaterTile((this.map.columns - 1), y);
            }

            // Set water Plane
            GameObject waterBackground = GameObject.Find("Background");
            waterBackground.GetComponent<SpriteRenderer>().size = new Vector2(this.map.columns + 30f, this.map.rows + 30f);
            waterBackground.transform.position = new Vector3(this.map.columns / 2, this.map.rows / 2, 4.0f);
        }

        private void SetNewWaterTile(int x, int y)
        {
            GameObject waterCullingTile = GameObject.Instantiate(this.map.waterTilePrefab, this.map.transform);
            GameObject waterTile = waterCullingTile.GetComponent<Culling>().tile;
            waterCullingTile.transform.SetParent(this.map.transform);

            waterCullingTile.transform.position = new Vector3(x, y, -0.01f);
            waterTile.GetComponent<Tile>().position = new Vector3(x, y, -0.01f);

            for (int i = 0; i < this.map.layer; i++)
            {
                GameObject tile = this.map.map[x, y, i];
                if (tile != null)
                {
        ...
        public List<GameObject> GetBorderedTiles()
        {
            ...
            borderedTiles.Add(map.map[0, 0, groundLayer]);
            borderedTiles.Add(map.map[0, map.rows - 1, groundLayer]);
            borderedTiles.Add(map.map[map.columns - 1, 0, groundLayer]);
            borderedTiles.Add(map.map[map.columns - 1, map.rows - 1, groundLayer]);

            for (int x = 1; x < (map.columns - 2); x++)
            {
                borderedTiles.Add(map.map[x, 0, groundLayer]);
                borderedTiles.Add(map.map[x, map.rows - 1, groundLayer]);
            }

            for (int y = 1; y < (map.rows - 2); y++)
            {
                borderedTiles.Add(map.map[0, y, groundLayer]);
                borderedTiles.Add(map.map[map.columns - 1, y, groundLayer]);
            }
        ...
        public List<GameObject> DeleteAllWaterTiles(List<GameObject> tileList)
        {
            ...
            for (int i = tileList.Count - 1; i >= 0; i--)
            {
                if (tileList[i] != null)
                {
                    ...
                    if (map.map[(int)tilePosition.x, (int)tilePosition.y, waterLayer] != null)
                    {
                        tileList.RemoveAt(i);
                    }
                }
        ...
        {
            byteMapping = null;
            GameObject field = map.map[x, y, z];
        ...
        public void GenerateGroundTransition(TileData tileData, List<GameObject> solidTileList)
        {
            ...
            foreach (GameObject solidCulledTile in solidTileList)
            {
                ...
                foreach (Vector2 neighborPosition in transitionNeighborsPosition)
                {
                    ...
                    GameObject field = map.map[x, y, layer];

                    if (field == null)
                    {
                        //Das �bergangsfeld ist leer und sollte vor der Manipulation erzeugt werden.
                        GameObject tilePattern = InstantiateToTilePattern(tileData);
        ...
        {
            // Du k�nntest hier auch StringBuilder verwenden, wenn die Leistung eine Rolle spielt
            string code = "";

            // Je nachdem, wie du die Reihenfolge w�nschst
        ...
        public ByteMapping UpdateByteMap(int x, int y, int layer)
        {
            ...
            // Top Left
            if ((x - 1) >= 0 && (y + 1) < map.map.GetLength(1))
            {
            ...
            // Top Right
            if ((x + 1) < map.map.GetLength(0) && (y + 1) < map.map.GetLength(1))
            {
                if (SaveToUse(x + 1, y + 1, layer, out tmpBuyteMap))
                {
            ...
            // Buttom Right
            if ((x + 1) < map.map.GetLength(0) && (y - 1) >= 0)
            {
                if (SaveToUse(x + 1, y - 1, layer, out tmpBuyteMap))
                {
